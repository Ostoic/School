using System.Collections;
using System.Collections.Generic;
using UnityEngine;

public class Level3Creation : MonoBehaviour {
	GameObject Play;
	GameObject Platform;
	GameObject nextNormalPlatform;
	GameObject nextHorizontalPlatform;
	GameObject nextVerticalPlatform;
	GameObject nextEnemyPlatform;
	GameObject floor;
		int jumpDist=3;
		int spawnPt=0;
		public static int endPt=10;

		int zoneCount=0;

		int nextPtx;
		int nextPty;
		int ceiling=6;
		int newSize;

		// Use this for initialization

		private int platformSize()
		{
			return (Random.Range(3, 6));
		}

		void newScene(int nextx,int nexty, int end){
			while (nextx < end) {

			if (Random.Range (1, 20) == 2) {
				Instantiate (Resources.Load ("Powerup"), new Vector3 (nextx, 2, 0), Quaternion.identity);
			}
			if (Random.Range (1, 20) == 2) {
				Instantiate (Resources.Load ("Collectible"), new Vector3 (nextx, 2, 0), Quaternion.identity);
			}


			if (Random.Range (1, 3) == 2) {
				newSize = platformSize();
				nextVerticalPlatform=(GameObject)	Instantiate (Resources.Load ("LevelDesign\\verticalPlatform"), new Vector3 (nextx + (newSize), nexty, 0), Quaternion.identity);
				nextVerticalPlatform.AddComponent<Level3OutOfBounds> ();
				nextVerticalPlatform.transform.localScale = new Vector3 (newSize, 1, 10);
				//Instantiate (Resources.Load ("VerticalPlatform"), new Vector3 (nextx, nexty, 0), Quaternion.identity);
			}
			else if (Random.Range (1, 3) == 2) {
				newSize = platformSize();
				nextHorizontalPlatform=(GameObject)	Instantiate (Resources.Load ("LevelDesign\\horizontalPlatform"), new Vector3 (nextx + (newSize), nexty, 0), Quaternion.identity);
				nextHorizontalPlatform.AddComponent<Level3OutOfBounds> ();
				nextHorizontalPlatform.transform.localScale = new Vector3 (newSize, 1, 10);
					//	Instantiate (Resources.Load ("horizontalPlatform"), new Vector3 (nextx, nexty, 0), Quaternion.identity);
			} 
			else { 

				newSize = platformSize();
				nextEnemyPlatform=(GameObject)Instantiate (Resources.Load ("LevelDesign\\EnemySpawnPlatform"), new Vector3 (nextx, nexty, 0), Quaternion.identity);
				nextEnemyPlatform.AddComponent<Level2OutOfBounds> ();
				nextEnemyPlatform.transform.localScale = new Vector3 (platformSize(), 1, 10);
			}
				nextx += Random.Range (spawnPt, spawnPt + (jumpDist+newSize*2));
				nexty = Random.Range (1, nexty + 4);
				if (nexty >= ceiling - 2) {
					nextPty = 5;
				}

			}
			nextPtx=nextx;
			nextPty=nexty;
			endPt+=10;
			zoneCount++;

			if (zoneCount == 10) {
				Instantiate(Resources.Load("LevelDesign\\levelComplete"), new Vector3(105, 1, 0), Quaternion.identity);
			}

		}
		void Start () {
		floor = (GameObject)Instantiate (Resources.Load ("LevelDesign\\Floor"), new Vector3 (0, 0, 0), Quaternion.identity);
		floor.transform.localScale = new Vector3 (5, 1, 10);
			Play=(GameObject.FindGameObjectWithTag("Player"));
		endPt = 10;
			nextPtx = spawnPt;
			nextPty = Random.Range (1, 3);

			newScene(nextPtx,nextPty,endPt);

		}

		// Update is called once per frame
		void Update () {
			Play=(GameObject.FindGameObjectWithTag("Player"));

			if (zoneCount<10&&Play.transform.position.x > (endPt - 13)) {
				newScene (nextPtx, nextPty, endPt);
			}
		}
	}